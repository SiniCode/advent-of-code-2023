...
class ReportReader:
    ...
    def find_previous_value(self, history):
        processing_steps = [history] + self.get_processing_steps(history, [])
        processing_steps.reverse()

        sums = [0]
        for step in processing_steps[1:]:
            sums.append(step[0] - sums[-1])

        return sums[-1]
    
    def calculate_sum_of_next_values(self):
        next_values = [self.find_previous_value(history) for history in self.histories]
        return sum(next_values)
    
if __name__ == '__main__':
    RR = ReportReader()
    print(RR.calculate_sum_of_next_values())  # correct: 889